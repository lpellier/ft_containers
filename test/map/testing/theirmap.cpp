...
void	_display_tree(const std::string & prefix, Node * _node, bool isLeft, bool childInRight) {
	if (_node) {
		std::cout << prefix;
...
{ 
	...
	for (int i = 0; i < 150; i++) {
		...
		if (i == 100)
			root = insert(root, 1800);
		if (i == 125)
			root = insert(root, 2400);
		root = insert(root, rand() % 150 + 1);
	}

	_display_tree("", root, false, (root->right ? true : false));
	cout << "is tree balanced ? " << (_is_balanced(root) ? "Yes" : "No") << endl;
    root = deleteNode(root, 666);
    cout << "is tree balanced ? " << (_is_balanced(root) ? "Yes" : "No") << endl;
   	root = deleteNode(root, 1200);
    cout << "is tree balanced ? " << (_is_balanced(root) ? "Yes" : "No") << endl;
    root = deleteNode(root, 1800);
    cout << "is tree balanced ? " << (_is_balanced(root) ? "Yes" : "No") << endl;
    root = deleteNode(root, 2400);
    cout << "is tree balanced ? " << (_is_balanced(root) ? "Yes" : "No") << endl;
